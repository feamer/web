...
function sha1(str) {
	...
	/*
	 * var lsb_hex = function (val) { // Not in use; needed? var str=""; var i;
	 * var vh; var vl;
	 * 
	 * for ( i=0; i<=6; i+=2 ) { vh = (val>>>(i*4+4))&0x0f; vl =
	 * (val>>>(i*4))&0x0f; str += vh.toString(16) + vl.toString(16); } return
	 * str; };
	 */
...
function login() {
	...
	var json_obj = '{' + '"username" : "'
			...
			+ '"password"  : "'
			+ sha1(document.getElementById('password_txtfield').value) + '"'
			+ '}';
	...
	/*
	 * $.post("http://51.144.0.67/login", json_obj, function(data) {
	 * console.log(data); });
	 */
	xhttp.onload = function() {
		window.location.replace("../connected/history");
	}
